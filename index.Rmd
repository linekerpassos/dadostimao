...
output: 
  flexdashboard::flex_dashboard:
    navbar:
      ...
      - { icon: "fa-question-circle", href: "https://www.meutimao.com.br/torcida/lp.88", align: right }
    theme:
      version: 4
      bg: "#dbdbdb"
      fg: "#101010" 
...
> Nota: valores ajustados pelo IPCA tomando como referência 2023.
...
ggplot(dados) + 
  ...
  scale_x_continuous(n.breaks = 11)+ # adiciona todos os anos no gráfico
  xlab("") + 
  ylab("")+
...
> Notas: (1) as demonstrações financeiras anteriores à 2019 não estão mais disponíveis no site do clube; (2) valores ajustados pelo IPCA tomando como referência 2023.

### Fluxo Receitas e Despesas 2023

```{r}
df_cor <- readxl::read_excel(
  "C:/Users/linek/Documents/R/dash_timao/plan-dre-timao.xlsx",
  sheet=1) %>% 
  mutate(type=case_when(
    tipo=="despesas_futebol"~"Receitas futebol",
    tipo=="receitas_futebol"~"Receitas futebol",
    tipo=="receitas_clubsocial"~"Receitas clube social",
    tipo=="despesas_clubsocial"~"Receitas clube social",
  )) %>% 
  filter(valor>0)

nodes = data.frame(c(df_cor$desc, df_cor$type)) %>%
  unique() %>% rename(node = 1)

df_cor_plot <-
df_cor %>% filter(tipo=="receitas_futebol"|tipo=="receitas_clubsocial") %>%
  # select(-type) %>%
  rename(Source = desc) %>%
  mutate(Target = type) %>%
  bind_rows(
    df_cor %>%
      filter(tipo=="despesas_futebol"|tipo=="despesas_clubsocial") %>%
      # select(-Category) %>%
      rename(Target = desc) %>%
      mutate(Source = type)
  ) %>% mutate(
    IDSource = match(Source, nodes$node)-1,
    IDTarget = match(Target, nodes$node)-1
  ) %>% as.data.frame() %>% 
  mutate(type=as.factor(type),
         type=fct_relevel(type,c("Receitas futebol","Receitas clube social"))) %>% 
  arrange(type) %>% 
  filter(valor>0)

my_color <- 'd3.scaleOrdinal() .domain(["black"]) .range(["black"])'

p <-
sankeyNetwork(Links = df_cor_plot, 
              Nodes = nodes,
              Source = "IDSource", 
              Target = "IDTarget",
              Value = "valor", 
...
htmlwidgets::onRender(p, '
  function(el) { 
    ...
    var labels = ["Receitas detalhadas", "Receita segmento", "Despesas"];
    cols_x.forEach((d, i) => {
...
> Notas: (1) valores no gráfico em milhares; (2) Receita Total: R$ 936.691 (em milhares); (3) Despesa Total: R$ 972.581 (em milhares).

### Fluxo Receitas e Despesas 2022
...
> Notas: (1) valores no gráfico em milhares; (2) Receita Total: R$ 779.103 (em milhares); (3) Despesa Total: R$ 784.867 (em milhares).

### Fluxo Receitas e Despesas 2021
...
> Notas: (1) valores no gráfico em milhares; (2) Receita Total: R$ 502.550 (em milhares); (3) Despesa Total: R$ 511.540 (em milhares).
